...
public class AdminDao {
	...
	public List<SellerVo>request(Connection conn, PageVo pvo) throws Exception {
		...
		// 쿼리문 
		String sql = "SELECT * FROM ( SELECT ROWNUM RNUM , A.* FROM ( SELECT * FROM SELLER S JOIN MEMBER M ON (S.MEMBER_NO = M.MEMBER_NO) WHERE REQUEST_QUIT_YN = 'Y' AND QUIT_YN = 'N' ORDER BY SELLER_NO ) A ) WHERE RNUM BETWEEN ? AND ?";
