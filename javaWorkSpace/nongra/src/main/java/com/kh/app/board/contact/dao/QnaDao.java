...
//	}
		...
		while(rs.next()) {
			...
			String categoryId = rs.getString("CATEGORY_ID");
			String categoryParentId = rs.getString("CATEGORY_PARENT_ID");
			String childCategoryName = rs.getString("CHILD_CATEGORY_NAME");
			...
			vo.setCategoryId(categoryId);
			vo.setCategoryParentId(categoryParentId);
			vo.setCategoryName(childCategoryName);
			vo.setCategoryName(parentCategoryName);
			vo.setMemberNo(memberNo);
			vo.setPayNo(payNo);
			vo.setTitle(title);
			vo.setContent(content);
	...
	//QNA 작성 소비자
	public int writeQnaMember(Connection conn, QnaMemberVo vo) throws Exception {
		String sql = "INSERT INTO QNA_MEMBER(MEMBER_QNA_NO, CATEGORY_ID, MEMBER_NO, PAY_NO, TITLE, CONTENT, ENROLL_DATE) VALUES (SEQ_QNA_MEMBER_NO.NEXTVAL, ?, ?, ?, ?, ?, SYSDATE)";
		PreparedStatement pstmt = conn.prepareStatement(sql);
